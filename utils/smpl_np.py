...
This is used to get the SMPL vertices from SMPL pose parameters
"""
import numpy as np
import pickle
import pyrender
...
if __name__ == '__main__':
  ...
  np.random.seed(9608)
  pose = (np.random.rand(*smpl.pose_shape) - 0.5) * 0.8
  beta = (np.random.rand(*smpl.beta_shape) - 0.5) * 0.06

  # print(smpl.trans_shape)
  trans = np.zeros(smpl.trans_shape)
  # trans = np.array([1.0100411176681519,-0.034865230321884155,-0.018879003822803497])
  smpl.set_params(beta=beta, pose=pose, trans=trans)

  smpl.save_to_obj('./smpl_np.obj')

  fuze_trimesh = trimesh.load('./smpl_np.obj')
