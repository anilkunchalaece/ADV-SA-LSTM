"""
Source - https://github.com/CalciferZh/SMPL
This is used to get SMPL vertices , joints and Rotation matrix representations for SMPL pose parametere
"""
import numpy as np
import pickle
import torch
...
class SMPLModel(Module):
  def __init__(self, device, model_path='./model.pkl'):
  ...
  def forward(self, betas, pose, trans, simplify=False):
    ...
    # print(pose.shape,pose.contiguous().view(-1, 1, 3).shape)

    R_cube_big = self.rodrigues(pose.contiguous().view(-1, 1, 3)).reshape(batch_num, -1, 3, 3)  # https://stackoverflow.com/questions/48915810/pytorch-contiguous
    ...
    # print(F"R_cube_big shape {R_cube_big.shape}")
    # print(F"R_cube {R_cube.shape}")
    joints = torch.tensordot(result, self.joint_regressor.transpose(1, 0), dims=([1], [0])).transpose(1, 2)

    return result.cpu().numpy(), joints.cpu().numpy(), R_cube_big.cpu().numpy()
...
def test_gpu(gpu_id):
  ...
  for i in range(2):
      pose = torch.from_numpy((np.random.rand(2, pose_size) - 0.5) * 0.4)\
              .type(torch.float64).to(device)
      betas = torch.from_numpy((np.random.rand(2, beta_size) - 0.5) * 0.06) \
              .type(torch.float64).to(device)
      # s = time()
      trans = torch.from_numpy(np.zeros((2, 3))).type(torch.float64).to(device)
      # print(betas.shape,pose.shape,trans.shape)
      result, joints, rotations = model(betas, pose, trans)
      print(result.shape,joints.shape,rotations.shape)

      # plt.scatter(joints[0,:,0],joints[0,:,1])
      # plt.scatter(joints[0,2,0],joints[0,2,1],marker='*',c='black')
      # x = [joints[0,2,0],joints[0,2,0]]
      # y = [joints[0,2,1],-1]
      # plt.plot(x,y)
      # x = [joints[0,2,0],joints[0,8,0]]
      # y = [joints[0,2,1],joints[0,8,1]]
      # plt.plot(x,y)

      # x = [joints[0,1,0],joints[0,1,0]]
      # y = [joints[0,1,1],-1]
      # plt.plot(x,y)
      # x = [joints[0,1,0],joints[0,7,0]]
      # y = [joints[0,1,1],joints[0,7,1]]
      # plt.plot(x,y)
      # plt.show()
      # print(F"min : {joints.min()} , max : {joints.max()}")
